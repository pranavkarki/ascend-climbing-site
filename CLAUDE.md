...
| `admin.html` / `admin.css` / `admin.js` | Password-protected CMS SPA at `/admin` (`.cms-*` scoped). Three tabs: Site Content (visual click-to-edit) + Menu + Field Notes. See **CMS** section below. |
...
| `api/` | Vercel serverless functions (Node, zero-config, no npm — built-in `crypto` + global `fetch` only). `_lib/` = github commit + auth + html/posts helpers; `login`/`logout`/`session` = auth; `content` = site-content GET/PUT; `menu` = cafe-menu GET/PUT; `posts` = blog GET/PUT; `upload` = cover-image commit. |
...
**Site Content editor (prices / timings / text — `index.html` only):**
- Editable values are marked in the HTML with **`data-cms="<key>"`** attributes. The API rewrites ONLY the inner text of those elements — surrounding markup/classes are never touched, so formatting can't break. **Elements all have plain-text inner content (no nested tags)**, so `api/_lib/content-html.js` uses an anchored regex (no DOM parser). Values are HTML-escaped on write / unescaped on read.
- **Visual click-to-edit is the default UI.** The admin fetches `/index.html` (`connect-src 'self'` allows it), strips every `<script>`, injects `<base href="/">` + editor CSS, and renders it into an **`<iframe srcdoc>`** — srcdoc is exempt from the site's `X-Frame-Options: DENY` / `frame-ancestors 'none'`, so no header changes were needed. Because scripts are stripped, the injected CSS must undo the pre-animation hidden states (`body{opacity:1}`, `.animate-up` reset) and hide `.nav-links`/`.menu-toggle` (JS-positioned). Each `[data-cms]` element gets its repo value (live HTML can lag a deploy), an outline, and click → `contenteditable="plaintext-only"` (paste-sanitizer fallback for old Firefox); Enter commits, Esc cancels, empty reverts. Edits accumulate in a pending map — nothing is sent until **Review & publish** (diff modal, per-row undo). The old grouped form remains behind the **"Edit as a list"** toggle (`localStorage cms-content-view`) as a fallback for fields a media query might hide.
- **Publish feedback (all tabs):** after PUT, `watchDeploy()` polls the public file (`/index.html`, `/cafe.html`, or `/posts.data.js`, `cache: no-store`, 15s × ~6 min) until it differs from a pre-publish snapshot (+ value check for content) → status goes "rebuilding…" → "Live on the site ✓". **Unsaved-edit guards:** every view registers a `dirtyCheck`; tab switches, logout and `beforeunload` all warn before dropping pending edits. All `confirm()`s were replaced by styled modals.
- **`cms-fields.js` is the single source of truth** mapping each `data-cms` key → label/group/`sub`/type/file. Fields are clustered by `sub` (e.g. "Boulder Pass", "Ages 6-8", "w/ Shoes") into separated blocks in the editor. To make a new element editable: add `data-cms="group.key"` in the HTML **and** a matching field entry in `cms-fields.js` — nothing else (both the visual editor and the list form pick it up automatically).
...
**Menu editor (its own "Menu" tab — `api/menu.js`):** the cafe menu is NOT in Site Content. It stays **static HTML in `cafe.html`** (good for SEO / no flash) inside a `<!-- cms:menu:start -->`…`<!-- cms:menu:end -->` region. `api/_lib/menu-html.js` parses that region into `[{ title, items: [{ name, price }] }]` on GET and regenerates it on PUT (preserving the page's indentation). The Menu tab lets the owner **edit name + price (✎ toggles inline inputs), add/remove items, add/remove sections** — supporting structural changes the `data-cms` text-only path can't. Publishing shows an added/removed-lines review diff (computed against the loaded snapshot) before the PUT. Do NOT add `data-cms` to menu rows; the whole region is regenerated.

**Field Notes (blog) editor:** edits `posts.data.js`. `/api/posts` GET parses it (runs it in a `vm` sandbox to capture `POSTS`/`BODIES`); PUT validates (required fields, unique integer `id`, `cat` ∈ event/story/rock, `DD.MM.YY` date, `details` keys ∈ allowed set) then **serializes the whole file as JSON-literal JS** and commits. The editor UI covers all post fields + `details` rows + optional long-form body (dek, caption, repeatable Section/Quote blocks with drop-cap, sign-off, tags) — with plain-language labels/hints, native `<input type="date">` pickers (converted to/from `DD.MM.YY` in `admin.js`), auto-estimated read time when left blank (~200 wpm), and inline field validation. **Preview** reuses `stories.html` itself: the admin fetches it, swaps the `posts.data.js` script tag for an inline `POSTS`/`BODIES` draft (with `</` escaped as `<`) + `location.hash = "#post-{id}"` set before the SPA boots, and shows it in a full-screen `srcdoc` iframe — pixel-perfect, zero duplicated rendering; a just-uploaded cover previews from its in-memory data URL since the commit hasn't deployed yet. Cover images: `admin.js` resizes to ≤1200px wide and re-encodes to **WebP via `<canvas>` client-side** (no server image tooling — AVIF recipe needs the CLI), then `/api/upload` commits it to `img/stories/{slug}.webp`. `imgTag` in `stories.html` already degrades gracefully if an image is missing.
