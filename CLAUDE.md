...
| `.testi-attrib` | Attribution row below quote: dash + uppercase name (`.testi-attrib-name`, 14px) + `✦` stars (green, `.testi-attrib-stars`) + muted "via Google · X ago" (`.testi-attrib-when`, 14px). Populated by JS. Uses `flex-wrap: wrap` — `.testi-attrib-when` has `width: 100%` to always force it onto its own line regardless of viewport width (prevents height variation between slides from shifting sections below) |
